...
public class TweetParser {
    ...
    static List<String> csvDataToTweets(BufferedReader br, int tweetColumn) {
        ...
        while (li.hasNext()) {
            String getTweet = extractColumn(li.next(), tweetColumn);
            if (getTweet != null && !getTweet.isEmpty()) {
                lines.add(getTweet);
            } else {
                return null;
            }
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        ...
        if (tweet != null) {
            for (String i: tweetSplit(removeURLs(tweet))) {
                List<String> sen = parseAndCleanSentence(i);
                if (!(sen.isEmpty())) {
                    cleaned.add(parseAndCleanSentence(i));
                }
            }
        }
        return cleaned;
    ...
    public static List<List<String>> csvDataToTrainingData(
        ...
        List<List<String>> trained = new LinkedList<>();
        for (String i: tweets) {
            trained.addAll(parseAndCleanTweet(i));
        }

        return trained; // Complete this method
    }
