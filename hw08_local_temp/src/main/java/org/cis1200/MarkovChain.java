...
public class MarkovChain implements Iterator<String> {
    ...
    void addBigram(String first, String second) {
        ...
        if (get(first) == null) {
            ProbabilityDistribution<String> prob = new ProbabilityDistribution<>();
            prob.record(second);
            chain.put(first, prob);
        } else {
            get(first).record(second);
        }
    ...
    public void train(Iterator<String> sentence) {
        ...
        this.currLine = sentence.next();
        startWords.record(currLine);
        while (sentence.hasNext()) {
            String next = sentence.next();
            addBigram(currLine, next);
            this.currLine = next;
        }
        addBigram(currLine, END_TOKEN);
    ...
    public String next() {
        if (hasNext()) {
            String first = currLine;
            try {
                this.currLine = get(first).pick(ng);
