...
public class SimpleManipulations {
    ...
    public static PixelPicture border(
        ...
        int w = pic.getWidth();
        int h = pic.getHeight();
        //resize by first making the entire pic the border color
        // and then adding the image on top of it
        Pixel[][] src = pic.getBitmap();
    ...
    public static PixelPicture invertColors(PixelPicture pic) {
        ...
        Pixel[][] invert = pic.getBitmap();
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                Pixel p = invert[row][col];
                int rInv = 255 - p.getRed();
                int gInv = 255 - p.getGreen();
                int bInv = 255 - p.getBlue();
                invert[row][col] = new Pixel(rInv, gInv, bInv);
            }
        }
        return new PixelPicture(invert);
    }
    ...
    public static PixelPicture grayScaleAverage(PixelPicture pic) {
        ...
        Pixel [][] grey = pic.getBitmap();
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                Pixel p = grey[row][col];
                int rGrey = p.getRed();
                int gGrey = p.getGreen();
                int bGrey = p.getBlue();
                int avg = (int) Math.round((rGrey + gGrey + bGrey) / 3.0);
                grey[row][col] = new Pixel(avg, avg, avg);
            }
        }
        return new PixelPicture(grey);
     ...
     * {@code 0 <= color <= 255}. ****CLARIFY THIS PART IN OFFICE HOURS****
     *
    ...
    public static PixelPicture scaleColors(
        ...
        Pixel [][] scale = pic.getBitmap();
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                Pixel p = scale[row][col];
                int rScale = (int) Math.round(p.getRed() * rfactor);
                int gScale = (int) Math.round(p.getGreen() * gfactor);
                int bScale = (int) Math.round(p.getBlue() * bfactor);

                scale[row][col] = new Pixel(rScale, gScale, bScale);
            }
        }
        return new PixelPicture(scale);
    }
    ...
    public static PixelPicture alphaBlend(
        ...
        int w = pic.getWidth();
        int h = pic.getHeight();
        int wSecondPic = f.getWidth();
        int hSecondPic = f.getHeight();

        Pixel [][] blend = pic.getBitmap();
        Pixel [][] blendSecondPic = f.getBitmap();
        if (w != wSecondPic || h != hSecondPic) {
            return pic;
        } else {
            for (int col = 0; col < w; col++) {
                for (int row = 0; row < h; row++) {
                    Pixel p = blend[row][col];
                    Pixel pSecondPic = blendSecondPic[row][col];
                    int rBlend = weightedAverage(alpha, p.getRed(), pSecondPic.getRed());
                    int gBlend = weightedAverage(alpha, p.getGreen(), pSecondPic.getGreen());
                    int bBlend = weightedAverage(alpha, p.getBlue(), pSecondPic.getBlue());
                    blend[row][col] = new Pixel(rBlend, gBlend, bBlend);
