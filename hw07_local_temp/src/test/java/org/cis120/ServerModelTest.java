...
public class ServerModelTest {
    ...
    public void testDeregisterOwner() {
        model.registerUser(0);
        Command createChannel = new CreateCommand(0, "User0", "channel1", false);
        createChannel.updateServerModel(model);
        Command createSecondChannel = new CreateCommand(1, "User1", "channel2", false);
        createSecondChannel.updateServerModel(model);
        model.deregisterUser(0);

        assertTrue(model.getChannels().contains("channel2"));
    }
    /*
    @Test
